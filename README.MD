...
4. If either postgres or redis is required than run command `docker-compose -f docker/external-services.yml up -d` Note: Only postgres or redis can be run by using command `docker-compose -f docker/external-services.yml up -d <NAME>` where `<NAME>` is replaced by either `db` or `redis` respectively.
